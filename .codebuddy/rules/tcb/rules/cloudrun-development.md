...
- 函数型：上手最快，内置 HTTP/WebSocket/SSE，端口固定 1890；本地运行受工具支持。
...
| 端口 | 固定 1890 | 应用自行监听 `PORT`（部署时由平台注入） |
...
4) 本地运行（仅函数型）
```json
{ "name": "manageCloudRun", "arguments": { "action": "run", "serverName": "my-svc", "targetPath": "/abs/ws/my-svc", "runOptions": { "port": 1890 } } }
```
...
7) 运行智能体（可选）
```json
{ "name": "manageCloudRun", "arguments": { "action": "run", "serverName": "my-agent", "targetPath": "/abs/ws/agents/my-agent", "runOptions": { "port": 1890, "runMode": "agent" } } }
```
