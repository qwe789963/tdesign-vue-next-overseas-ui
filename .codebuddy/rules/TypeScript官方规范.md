...
- 优先考虑代码的可读性和可维护性
- 遵循一致性原则，保持代码风格统一
- **【必须】遵循 `team-rule.md` 中的命名规范和代码规范**
...
## 12. 命名规范【必须】
- **【必须】遵循 team-rule.md 中的命名规范**
- 变量和函数使用驼峰命名法(camelCase)
- 类和构造函数使用帕斯卡命名法(PascalCase)
- 导出的常量使用全大写(UPPER_SNAKE_CASE)
- 避免使用前置或后置下划线
...
class UserManager {}
export const API_KEY: string = 'secret';
...
- **【必须】所有变量、参数、返回值都必须有明确的类型声明**（与 team-rule.md 保持一致）
