 ...
 *
 * @description
 * 管理 Radio 组件的 focus/blur 视觉反馈（Focus 外圈显示/隐藏）
...
export function useFocusHandler(): UseFocusHandlerReturn {
   ...
   *
   * @description
   * - 为 .t-radio__input 元素添加 'focusInput' 类名
   * - 移除 'normalInput' 类名
   * - 触发 .focusBox 元素显示 (通过 CSS display: inline-block)
   *
   * @remarks
   * - **完全复刻 Vue2 源码**：packages/overseas/src/radio/radio.tsx 第 111-117 行
   * - 包括看似"无意义"的循环（实际上可能是为了处理某种竞态条件）
   * - Vue2 实现：this.$el.children[1].classList.add('focusInput')
   *
   * @example
   * ```tsx
  ...
  const handleFocus = (): void => {
    ...
    const radioDom = document.getElementsByClassName('t-radio__former');

    for (let i = 0; i < radioDom.length; i++) {
      // Vue2 实现：每次循环都操作同一个元素 this.$el.children[1]
      const radioInput = focusInputRef.value.children[1] as HTMLElement;
      if (!radioInput) {
        continue;
      }
   ...
   *
   * @description
   * - 为 .t-radio__input 元素添加 'normalInput' 类名
   * - 移除 'focusInput' 类名
   * - 触发 .focusBox 元素隐藏 (通过 CSS display: none)
   *
   * @remarks
   * - **完全复刻 Vue2 源码**：packages/overseas/src/radio/radio.tsx 第 118-124 行
   * - 包括看似"无意义"的循环（实际上可能是为了处理某种竞态条件）
   * - Vue2 实现：this.$el.children[1].classList.add('normalInput')
   *
   * @example
   * ```tsx
